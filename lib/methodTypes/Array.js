...
const arrayUnique2 = (arr, name) =>
  ((myMap) => arr.filter((item) => !myMap.has(item.name) && myMap.set(item.name, true)))(new Map());
...
const hasSingleTarget = (list, target, key) => {
  const arr = list.filter((item) => (key ? item[key] === target[key] : item === target));
 ...
 * @return {boolean}
 */
const hasAllTargets = (targets, src, key) => {
  let intersectionLists = [];
  if (key) {
    const ids = targets.map((item) => item[key]);
    intersectionLists = src.filter((v) => ~ids.indexOf(v[key]));
  } else {
    intersectionLists = [...new Set([...targets].filter((x) => new Set(src).has(x)))];
  }
